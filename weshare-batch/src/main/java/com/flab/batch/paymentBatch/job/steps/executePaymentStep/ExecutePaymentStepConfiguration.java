package com.flab.batch.paymentBatch.job.steps.executePaymentStep;
...
public class ExecutePaymentStepConfiguration {
	...
	@Bean
	@StepScope
	public ZeroOffsetJpaPagingItemReader<Payment> waitingPaymentItemReader() {
		return new ZeroOffsetJpaPagingItemReaderBuilder<Payment>().name("waitingPaymentItemReader")
			.entityClass(Payment.class)
			.queryString("select pm from Payment pm "
				+ "join fetch pm.card "
				+ "left outer join fetch pm.payResult "
				+ "where pm.payDate=:payDate")
			.parameterValues(Map.of("payDate", parameter.getPayJobDate()))
			.pageSize(CHUNKSIZE)
			.build();
	}
